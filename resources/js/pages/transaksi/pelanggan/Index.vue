...
const props = defineProps<CustomerPageProps>();
const customerStore = useCustomerStore();

onMounted(() => {
    customerStore.initializeFromProps(props);
});
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="flex items-center justify-between gap-4 mb-3">
                            <!-- Search Bar -->
                            <div class="flex-grow">
                                <SearchInput v-model="customerStore.searchQuery"
                                    placeholder="Cari berdasarkan nama pelanggan..."
                                    @update:modelValue="customerStore.setSearchQuery" />
                            </div>

                            <Link :href="route('admin.transaksi.customer.create')"
                        ...
                        <Table v-else class="min-w-full bg-white shadow-md rounded-lg overflow-hidden dark:bg-gray-900">
                            ...
                            <template #body>
                                <tr v-if="customerStore.customers.data.length === 0">
                                    <td colspan="5" class="px-4 py-10 text-center">
                                        <div
                                            ...
                                            <p class="text-sm">Tidak ada data pelanggan yang ditemukan.</p>
                                            <button @click="customerStore.setSearchQuery('')"
                                ...
                                <tr v-for="(customer, index) in customerStore.customers.data" :key="customer.slug"
                                    ...
                                    <td class="px-4 py-1.5 text-gray-900 dark:text-gray-100">{{ index + 1 +
                                        (customerStore.customers.current_page - 1) * customerStore.customers.per_page }}
                                    </td>
                                    <td class="px-4 py-1.5 text-gray-900 dark:text-gray-100">{{ customer.name }}</td>
                                    <td class="px-4 py-1.5 text-gray-900 dark:text-gray-100">{{ customer.phone ? customer.phone : '-' }}</td>
                                    <td class="px-4 py-1.5">
                                        <Badge :type="customer.is_active ? 'success' : 'danger'"
                                            :text="customer.is_active ? 'Aktif' : 'Nonaktif'" />
                                    </td>
                                    <td class="px-2 py-1 text-right">
                                        <div class="flex justify-end gap-2">
                                            <Link :href="route('admin.transaksi.customer.edit', customer.slug)"
                                            ...
                                            <button @click="customerStore.deleteCustomer(customer)"
                        ...
                        <!-- Pagination -->
                        <Pagination :links="customerStore.customers.links"
                            :current-page="customerStore.customers.current_page"
                            :per-page="customerStore.customers.per_page" :total="customerStore.customers.total" />
                    </div>
                </div>
            </div>
        </div>
