...
import { ProductPageProps } from '@/types/produk';
...
onMounted(() => {
    productStore.initializeFromProps(props);
});

watch(() => props.products, (newProducts) => {
    productStore.products = newProducts;
}, { deep: true });
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="flex items-center justify-between gap-4 mb-3">
                            <!-- Search Bar -->
                            <div class="flex-grow">
                                <SearchInput v-model="productStore.searchQuery"
                                    placeholder="Cari berdasarkan kode atau nama produk..."
                                    @update:modelValue="productStore.setSearchQuery" />
                            </div>

                            <Link :href="route('admin.master.product.create')"
                        ...
                        <Table v-else class="min-w-full bg-white shadow-md rounded-lg overflow-hidden overflow-x-auto dark:bg-gray-900">
                            <template #header>
                                <tr class="text-gray-900 dark:text-gray-800">
                                    ...
                                    <th class="px-4 py-2 text-left text-sm font-semibold">Varian</th>
                                    <th class="px-4 py-2 text-left text-sm font-semibold">Satuan Dasar</th>
                                    <th class="px-4 py-2 text-left text-sm font-semibold">Harga Beli</th>
                                    <th class="px-4 py-2 text-left text-sm font-semibold">Harga Jual</th>
                                    <th class="px-4 py-2 text-left text-sm font-semibold">Status</th>
                                    <th class="px-4 py-2 text-right text-sm font-semibold">Aksi</th>
                                </tr>
                            </template>
                            <template #body>
                                <tr v-if="productStore.products.data.length === 0">
                                    <td colspan="9" class="px-4 py-10 text-center">
                                        <div
                                            ...
                                            <button @click="productStore.setSearchQuery('')"
                                ...
                                <tr v-for="(product, index) in productStore.products.data" :key="product.slug"
                                    ...
                                    <td class="px-4 py-1.5 text-gray-900 dark:text-gray-100">{{ index + 1 +
                                        (productStore.products.current_page - 1) * productStore.products.per_page }}
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">
                                            <div class="flex flex-col">
                                                <span class="font-medium">{{ product.name }}</span>
                                                <span class="text-xs text-gray-500 dark:text-gray-400">Kode: {{ product.code }}</span>
                                            </div>
                                        </td>
                                    <td class="px-4 py-1.5 text-gray-900 dark:text-gray-100">{{ product.variant_name }}
                                    </td>
                                    <td class="px-4 py-1.5 text-gray-900 dark:text-gray-100">{{
                                        product.defaultUnit?.name }}</td>
                                    <td class="px-4 py-1.5 text-gray-900 dark:text-gray-100">{{ formatNumber(product.purchase_price)
                                        }}</td>
                                    <td class="px-4 py-1.5 text-gray-900 dark:text-gray-100">{{ formatNumber(product.selling_price) }}
                                    </td>
                                    <td class="px-4 py-1.5">
                                        <Badge :type="product.is_active ? 'success' : 'danger'"
                                            :text="product.is_active ? 'Aktif' : 'Nonaktif'" />
                                    </td>
                                    <td class="px-2 py-1 text-right">
                                        <div class="flex justify-end gap-1">
                                            <Link :href="route('admin.master.product.show', product.slug)"
                                            ...
                                            <Link :href="route('admin.master.product.edit', product.slug)"
                                            ...
                                            <button @click="productStore.deleteProduct(product)"
                        ...
                        <!-- Pagination -->
                        <Pagination :links="productStore.products.links"
                            :current-page="productStore.products.current_page"
                            :per-page="productStore.products.per_page" :total="productStore.products.total" />
                    </div>
                </div>
            </div>
        </div>
