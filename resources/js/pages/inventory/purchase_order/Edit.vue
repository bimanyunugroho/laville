...
function createExistDetail(detail: PurchaseOrderDetail): PurchaseOrderDetail {
    return {
...
const initialDetails = props.purchase_order.details && props.purchase_order.details.length > 0
    ? props.purchase_order.details.map(createExistDetail)
    : [createEmptyDetail()];

const formEditPurchaseOrder = ref({
    po_number: props.purchase_order.po_number,
    supplier_id: props.purchase_order.supplier_id,
    user_id: props.purchase_order.user_id,
    user_ack_id: null,
    user_reject_id: null,
    po_date: props.purchase_order.po_date,
    expected_date: props.purchase_order.expected_date,
    ack_date: null,
    reject_date: null,
    subtotal: props.purchase_order.subtotal,
    tax: props.purchase_order.tax,
    discount: props.purchase_order.discount,
    total_net: props.purchase_order.total_net,
    status: props.purchase_order.status,
    notes: props.purchase_order.notes ?? null,
...
// Menambah item baru
function addItem() {
    formEditPurchaseOrder.value.details.push(createEmptyDetail());
}

// Menghapus item
function removeItem(index: number) {
    if (formEditPurchaseOrder.value.details.length > 1) {
        formEditPurchaseOrder.value.details.splice(index, 1);
        calculateTotals();
    }
}
...
function handleProductChange(index: number) {
    const detail = formEditPurchaseOrder.value.details[index];
    const product = findProduct(detail.product_id);

    if (product && product.defaultUnit) {
        formEditPurchaseOrder.value.details[index].unit_id = product.defaultUnit.id;
        formEditPurchaseOrder.value.details[index].price = product.purchase_price || 0;
...
function calculateTotals() {
    formEditPurchaseOrder.value.subtotal = formEditPurchaseOrder.value.details.reduce((sum, detail) => sum + detail.subtotal, 0);
    const discountAmount = formEditPurchaseOrder.value.discount || 0;
    const subtotalWithDiscount = formEditPurchaseOrder.value.subtotal - discountAmount;
    const taxAmount = formEditPurchaseOrder.value.tax ? (subtotalWithDiscount * (formEditPurchaseOrder.value.tax / 100)) : 0;
    formEditPurchaseOrder.value.total_net = subtotalWithDiscount + taxAmount;
    formEditPurchaseOrder.value.taxAmount = taxAmount;
}

// Handle submit form
function submitForm() {
    purchaseOrderStore.editPurchaseOrder(props.purchase_order.slug, formEditPurchaseOrder.value);
}
                        ...
                        <form @submit.prevent="submitForm">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                ...
                                <div class="space-y-6">
                                    ...
                                    <!-- Nomor PO -->
                                    <div class="mt-4">
                                        <TextInput id="po_number" v-model="formEditPurchaseOrder.po_number" label="No.PO"
                                            placeholder="Masukkan No.PO" required :readonly="true" />
                                    </div>

                                    <!-- Supplier -->
                                    <div>
                                        <VSelect id="supplier_id" v-model="formEditPurchaseOrder.supplier_id" :options="supplierOptions"
                                    ...
                                    <div class="flex items-center justify-between mb-2">
                                        <!-- Tanggal PO -->
                                        <DateInput id="po_date" v-model="formEditPurchaseOrder.po_date" label="Tanggal PO"
                                            placeholder="Masukkan Tanggal PO" required class="w-full sm:w-[48%]" />

                                        <!-- Tanggal Expektasi PO di sebelah kanan -->
                                        <DateInput id="po_date" v-model="formEditPurchaseOrder.expected_date"
                                ...
                                <div class="space-y-6">
                                    <!-- Catatan -->
                                    <div>
                                        <TextAreaInput v-model="formEditPurchaseOrder.notes" id="notes" label="Catatan"
                                            placeholder="Masukkan Catatan" :rows="18" :maxlength="900"
                                            :required="false" />
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="overflow-x-auto mb-2">
                                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                                        ...
                                        <tbody
                                            ...
                                            <tr v-for="(detail, index) in formEditPurchaseOrder.details" :key="index">
                                                <td class="px-4 py-2 w-64">
                                                    <VSelect id="product_id" v-model="formEditPurchaseOrder.details[index].product_id"
                                                ...
                                                <td class="px-4 py-2 w-64">
                                                    <VSelect v-model="formEditPurchaseOrder.details[index].unit_id"
                                                ...
                                                <td class="px-4 py-2">
                                                    <TextNumber id="quantity"
                                                        v-model.number="formEditPurchaseOrder.details[index].quantity"
                                                        @input="calculateBaseQuantity(index)" />
                                                </td>

                                                <td class="px-4 py-2">
                                                    <TextNumber id="price" v-model="formEditPurchaseOrder.details[index].price"
                                                ...
                                                <td class="px-4 py-2">
                                                    <button type="button" @click="removeItem(index)"
                                                        ...
                                                        :disabled="formEditPurchaseOrder.details.length <= 1">
                                    ...
                                    <div class="w-full md:w-1/3">
                                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow-md">
                                            ...
                                            <div class="flex justify-between items-center mb-2">
                                                ...
                                                <span class="font-medium">{{ formatCurrency(formEditPurchaseOrder.subtotal) }}</span>
                                            </div>
                                            ...
                                            <div class="mb-4">
                                                <div class="flex justify-between items-center mb-1">
                                                    ...
                                                    <div class="w-1/2">
                                                        <TextNumber id="discount" v-model.number="formEditPurchaseOrder.discount"
                                            ...
                                            <div class="mb-2">
                                                <div class="flex justify-between items-center mb-1">
                                                    ...
                                                    <div class="flex items-center w-1/2 space-x-2">
                                                        <TextNumber id="tax" v-model.number="formEditPurchaseOrder.tax"
                                                ...
                                                <div class="flex justify-between items-center mt-1">
                                                    ...
                                                    <span
                                                        ...
                                                        {{ formatCurrency(formEditPurchaseOrder.taxAmount) }}
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <div
                                                ...
                                                <span class="font-bold text-lg text-blue-600 dark:text-blue-400">
                                                    {{ formatCurrency(formEditPurchaseOrder.total_net) }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="flex items-center justify-end space-x-3 pt-4">
                                ...
                                <BaseButton :loading="purchaseOrderStore.isLoading" text="Ubah"
