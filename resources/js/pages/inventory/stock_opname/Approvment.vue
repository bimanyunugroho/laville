...
const formValidateStockOpname = ref(<StockOpnameValidate>{
    ...
    reject_date: null,
    status: null,
    is_locked: props.stock_opname.is_locked
});

const statusOptions = computed(() => {
    return props.status_so
        ?.filter(status => status.label !== 'DRAFT' && status.label !== 'VALIDATED')
        .map(status => ({
            id: status.label,
            name: status.name
        })) || [];
...
watch(() => formValidateStockOpname.value.status, (newStatusId) => {
    ...
    if (newStatusId === 'COMPLETED') {
        formValidateStockOpname.value.user_ack_id = props.currentUser.id;
        formValidateStockOpname.value.ack_date = now;
        ...
        formValidateStockOpname.value.user_ack_id = null;
        formValidateStockOpname.value.ack_date = null;
        formValidateStockOpname.value.is_locked = true;
    } else {
        formValidateStockOpname.value.user_ack_id = null;
        formValidateStockOpname.value.ack_date = null;
...
async function approvalForm() {
    ...
    if (result.isConfirmed) {
        try {
            ...
            await stockOpnameStore.approvalStockOpname(
                                ...
                                <div
                                    ...
                                    <div class="border-b border-gray-200 dark:border-gray-600 pb-4 mb-4">
                                        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                                            Form Approval Stock Opname
                                        </h3>
                                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                            Silakan pilih status Approval untuk dokumen ini
                                        </p>
                                    </div>
                                    ...
                                    <div v-if="formValidateStockOpname.status"
                                        ...
                                        <div class="flex items-center gap-2">
                                            <span
                                                ...
                                                :class="{
                                                    'bg-emerald-100 text-emerald-800 border border-emerald-300': formValidateStockOpname.status === 'COMPLETED',
                                                    'bg-red-100 text-red-800 border border-red-300': formValidateStockOpname.status === 'CANCELED',
                                                    'bg-blue-100 text-blue-800 border border-blue-300': formValidateStockOpname.status !== 'COMPLETED' && formValidateStockOpname.status !== 'CANCELED',
                                                }">
                                                <template
                                                    v-if="formValidateStockOpname.status === 'COMPLETED'">
                                        ...
                                        <div v-if="formValidateStockOpname.user_ack_id && formValidateStockOpname.ack_date"
                                            class="flex items-center gap-3 p-4 rounded-lg border border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/20 w-fit">
                                            <div class="flex flex-col items-center justify-center gap-2">
                                                <User2 class="w-5 h-5 text-emerald-500 dark:text-emerald-400" />
                                                <Calendar class="w-5 h-5 text-emerald-500 dark:text-emerald-400" />
                                            </div>
                                            <div class="text-sm text-emerald-700 dark:text-emerald-400">
                                                ...
                                                <div class="italic">{{ formValidateStockOpname.ack_date }}
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div v-if="!formValidateStockOpname.status"
                                        ...
                                        <div class="flex items-start gap-3">
                                            ...
                                            <div class="text-sm text-blue-700 dark:text-blue-300">
                                                ...
                                                <p class="mt-1">Silakan pilih status approvment untuk melanjutkan proses Stock Opname</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    dark:hover:bg-gray-600 transition-colors">
                                        ...
                                        <BaseButton :loading="stockOpnameStore.isLoading"
                                            :text="formValidateStockOpname.status === 'COMPLETED' ? 'APPROVMENT' : formValidateStockOpname.status === 'CANCELED' ? 'REJECTED' : 'COMPLETED'"
                                            loadingText="Proses Approvment..."
                                            :color="formValidateStockOpname.status === 'COMPLETED' ? 'emerald' : formValidateStockOpname.status === 'CANCELED' ? 'red' : 'blue'"
