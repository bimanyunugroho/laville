...
import { Calendar, Search, Package, Loader2, Percent, AlertTriangle } from 'lucide-vue-next';
import { formatCurrency } from '@/helpers/currencyHelper';
import TextNumber from '@/components/TextNumber.vue';
import { useGoodReceiptStore } from '@/stores/goodReceiptStore';
import { GoodReceiptDetail } from '@/types/good_receipt';
...
const searchLoading = computed(() => goodReceiptStore.purchaseOrderSearchLoading);
const poSearchResult = computed(() => goodReceiptStore.purchaseOrderResult);
const tanggalPenerimaanBarang = new Date().toISOString().split('T')[0];
const isSubmitting = ref(false);
const hasModifieddetails = ref(false);

const form = ref({
    receipt_number: props.receipt_number,
    purchase_order_id: 0,
    supplier_id: 0,
    user_id: props.currentUser?.id || 0,
    receipt_date: tanggalPenerimaanBarang,
    user_ack_id: null,
    ack_date: null,
    user_reject_id: null,
    reject_date: null,
    ...
    taxAmount: 0,
    total_net: 0,
    status_receipt: 'PROSESS',
    notes: '',
    is_active: true,
    supplier_name: '',
    po_number: '',
    details: [] as GoodReceiptDetail[]
});

const populateFormFromPO = () => {
    if (!poSearchResult.value) {
        return;
    }

    const po = poSearchResult.value;

    form.value.purchase_order_id = po.id || 0;
    form.value.supplier_name = po.supplier?.name || '';
    form.value.po_number = po.po_number || '';
    form.value.supplier_id = po.supplier_id || 0;
    form.value.discount = po.discount || 0;
    form.value.tax = po.tax || 0;

    // Map PO details to receipt details
    if (po.details && Array.isArray(po.details) && po.details.length > 0) {
        form.value.details = po.details.map(detail => {
            return {
                id: detail.id || 0,
                good_receipt_id: null,
                purchase_order_detail_id: detail.id || 0,
                product_id: detail.product_id || 0,
                unit_id: detail.unit_id || 0,
                quantity: detail.quantity || 0,
                base_quantity: detail.base_quantity || 0,
                received_quantity: detail.quantity || 0, // Default to full quantity
                received_base_quantity: detail.base_quantity || 0,
                price: detail.price || 0,
                subtotal: (detail.quantity || 0) * (detail.price || 0),
                created_at: null,
                unit: detail.unit || undefined,
                product: detail.product || undefined
            } as typeof form.value.details[number];
        });
    } else {
        form.value.details = [];
    }
...
const clearPOSearch = () => {
    goodReceiptStore.clearPurchaseOrderSearch();
    form.value.purchase_order_id = 0;
    form.value.po_number = '';
    form.value.supplier_id = 0;
    form.value.supplier_name = '';
    form.value.details = [];
    form.value.subtotal = 0;
    form.value.total_net = 0;
    form.value.taxAmount = 0;
    hasModifieddetails.value = false;
};

const submitForm = () => {
    isSubmitting.value = true;
    calculateTotals();
    goodReceiptStore.createGoodReceipt(form.value);
};
...
watch(() => poSearchResult.value, (newValue) => {
    if (newValue) {
        populateFormFromPO();
    } else {
        goodReceiptStore.purchaseOrderQuery = '';
    }
});
</script>
                    ...
                    <div class="p-6">
                        ...
                        <div
                            ...
                            <h2 class="text-lg font-medium text-gray-900 dark:text-white mb-4 flex details-center">
                                ...
                                Cari Nomor Purchase Order
                            </h2>

                            <div class="flex flex-wrap gap-4 mb-4">
                                <div class="flex-1 min-w-[250px]">
                                    <TextInput v-model="goodReceiptStore.purchaseOrderQuery" id="purchase-order-query"
                                        label="Nomor PO" placeholder="Masukkan nomor PO"
                                        @update:modelValue="goodReceiptStore.setPurchaseOrderQuery($event)"
                                        :disabled="!!form.purchase_order_id" />
                                </div>
                                <div class="flex details-end">
                                    <BaseButton v-if="!form.purchase_order_id" text="Cari" :loading="searchLoading"
                                        loadingText="Mencari..." @click="goodReceiptStore.searchNoPurchaseOrder()"
                                        color="blue" />
                                    <BaseButton v-else text="Ganti PO" @click="clearPOSearch" color="gray" />
                                </div>
                            </div>
                            ...
                            <!-- No PO found message -->
                            <div v-else-if="goodReceiptStore.purchaseOrderQuery && !searchLoading && !poSearchResult"
                                class="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 rounded-md border border-yellow-200 dark:border-yellow-800 flex details-center">
                                <AlertTriangle class="w-5 h-5 mr-2" />
                                <span>Nomor PO tidak ditemukan. Silakan periksa kembali nomor PO Anda.</span>
                            </div>
                        </div>
                        ...
                        <form @submit.prevent="submitForm">
                            ...
                            <div
                                ...
                                <BaseButton
                                    ...
                                    :loading="isSubmitting || goodReceiptStore.isLoading" text="Simpan Penerimaan"
                                    loadingText="Menyimpan..." color="blue" icon="save" />
                            </div>
                        </form>
                    </div>
                </div>
